    ...
    initD3() {
        ...
        _this.$nextTick(() => {
          ...
          for (let i = 0; i < resData.length; i++) {
            edgeResult[i] = {
              id: resData[i].id,
              source:
                resData[i].direction === 0
                  ? node.id
                  : resData[i].relation_node.id,
              target:
                resData[i].direction === 0
                  ? resData[i].relation_node.id
                  : node.id,
      ...
      function deleteNextNodes(node) {
        ...
        tempNodeList.forEach(function (item) {
          const hasLine = relationList.findIndex(
      ...
      function toggleMenu(current, d, flag) {
        ...
        labelsVals
          ...
          .attr('startOffset', function (d, i) {
            ...
            const angle = pieAngle[i]
            const percent =
              ((p.startAngle + p.endAngle) / 2 / 2 / Math.PI) * 100
            if (angle > 90 && angle <= 270) {
              // 分别计算每条路径的正确百分比
              return 100 - percent + '%'
            }
      ...
      function textBreaking(dom, data, breaking) {
        ...
        if (breaking) {
          ...
          if (len <= 3) {
            dom.append('tspan').attr('x', 0).attr('y', 0).text(text)
          } else {
